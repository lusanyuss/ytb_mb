    ...
    public void setCurrentItem(int item, boolean anim) {
        ...
        if (mSelectedTabIndex != item) {
            ...
            for (int i = 0; i < tabCount; i++) {
                ...
                TextView child = (TextView) group.getChildAt(0);
            ...
            // if (scrollBar != null && mPositionOffset < 0.01f &&
            // mPreSelectedTabIndex >= 0 && mPreSelectedTabIndex <
            // getChildCount()) {
            // // 获取scrollBar开始移动的起始位置
            // int sx = getChildAt(mPreSelectedTabIndex).getLeft();
            // // 获取scrollBar结束的位置
            // int ex = getChildAt(item).getLeft();
            // final float pageDelta = (float) Math.abs(ex - sx) /
            // (getChildAt(item).getWidth());
