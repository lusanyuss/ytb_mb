...
public class IndicatorViewPager {
    ...
    private OnItemSelectedListener onItemSelectedListener = new OnItemSelectedListener() {
        ...
        public void onItemSelected(View selectItemView, int select, int preSelect) {
            // activity.up.setVisibility(View.GONE);
    ...
    private OnPageChangeListener onPageChangeListener = new OnPageChangeListener() {
        ...
        public void onPageSelected(final int position) {
            ...
            if (menu != null) {
                if (position == 0) {
                    menu.setMode(SlidingMenu.LEFT); // 第一个。SlidingMenu仅有左边有效
    ...
    public IndicatorViewPager(Activity activity, Indicator indicator, ViewPager viewPager) {
        super();
    ...
    /**
     * 设置缓存界面的个数，左右两边缓存界面的个数，不会被重新创建。<pt> 默认是1，表示左右两边
    ...
    /**
     * 设置预加载界面的个数。左右两边加载界面的个数<pt> 默认是1，表示左右两边 相连的1个界面会和当前界面同时加载
     *
     * @param limit
     */
